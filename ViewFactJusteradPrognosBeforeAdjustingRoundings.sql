...
/****** Object:  View [dbo].[ViewFactFteJusteradPrognos]    Script Date: 2025-09-23 09:13:34 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW  [dbo].[ViewFactFteJusteradPrognos]
AS
WITH src AS (
    SELECT
        ...
        vf.[Kostnadsställe]                          AS Kostnadsstalle,

        vf.[Månadslön]            AS Manadslon,
        vf.[Lönetillägg]          AS Lonetillagg,
        vf.[MånadsLönPlusTillägg] AS ManadslonPlusTillagg,
        vf.[Timlön]               AS Timlon,
        vf.[AktualitetId]         AS AktualitetId,         -- 1=Utfall, 2=Prognos

        vf.[FTE Beräkning]        AS Kalla,                -- ★ NYTT: Källa (TimRapportering / KontraktuellFte)
...
weighted AS (
    SELECT
        ...
        Lonekategori,
        Kostnadsstalle,
        -- ★ NYTT: Exkludera kontraktuell FTE endast i PROGNOS för Intermittent/Särskild visstid
        CASE 
            WHEN AktualitetId = 2
             AND Kalla = N'KontraktuellFte'
             AND (Anstallningsform LIKE N'Intermittent%' OR Anstallningsform LIKE N'Särskild visstidsanställning%')
            THEN 1 ELSE 0
        END AS ExcludePrognos,

        -- Effektiva FTE-värden
        CASE 
            WHEN AktualitetId = 2
             AND Kalla = N'KontraktuellFte'
             AND (Anstallningsform LIKE N'Intermittent%' OR Anstallningsform LIKE N'Särskild visstidsanställning%')
        ...
        CASE 
            WHEN AktualitetId = 2
             AND Kalla = N'KontraktuellFte'
             AND (Anstallningsform LIKE N'Intermittent%' OR Anstallningsform LIKE N'Särskild visstidsanställning%')
            THEN 0.0 ELSE FTE_ForeJustering
        END AS FTE_ForeJustering_eff,
        CASE 
            WHEN AktualitetId = 2
             AND Kalla = N'KontraktuellFte'
             AND (Anstallningsform LIKE N'Intermittent%' OR Anstallningsform LIKE N'Särskild visstidsanställning%')
            THEN 0.0 ELSE FTE_FranvaroJustering
        END AS FTE_FranvaroJustering_eff,
        ...
        -- Rå vikt och justerad vikt
        CASE 
            WHEN Lonekategori = N'Timavlönad' OR Timlon IS NOT NULL THEN WorkedFlag
            ELSE FractionMonth
        END AS RowWeightRaw,
...
SELECT
    ...
    /* 🔹 FTE (månadsmedel) – använder _eff */
    CAST(SUM(w.FTE_ForeJustering_eff)     / 12.0 AS decimal(14,2)) AS [MedelFTEFöreJustering],
    CAST(SUM(w.FTE_FranvaroJustering_eff) / 12.0 AS decimal(14,2)) AS [MedelFTEFrånvaroJustering],
    CAST(SUM(w.FTE_eff)                   / 12.0 AS decimal(14,2)) AS [MedelFTE],

    CAST(SUM(w.RowWeight) AS decimal(14,2)) AS AntalMånaderEnligtKontrakt,

    CAST(SUM(
        CASE 
            -- TIM: justera månaden med FTE_eff (klamp 0..1)
            WHEN w.Lonekategori = N'Timavlönad' OR w.Timlon IS NOT NULL
                THEN w.RowWeight * CASE WHEN w.FTE_eff < 0 THEN 0 WHEN w.FTE_eff > 1 THEN 1 ELSE w.FTE_eff END
            -- MÅNAD: kvot av eff-värden (klamp 0..1)
            ELSE w.RowWeight *
                 CASE 
                    WHEN w.FTE_ForeJustering_eff <= 0 THEN 0
                    WHEN w.FTE_eff < 0 THEN 0
                    WHEN w.FTE_eff > w.FTE_ForeJustering_eff THEN 1
                    ELSE w.FTE_eff / w.FTE_ForeJustering_eff
                 END
        END
    ...
    /* ✅ Effektiva satser (årsslut om finns, annars årsmedel) + källa */
